...
nLinesBefore++;nLinesAfter++;var regexText;var replacementText;var match;match=/(^\n*)/.exec(this.selection);this.selection=this.selection.replace(/(^\n*)/,"");this.startTag=this.startTag+(match?match[1]:"");match=/(\n*$)/.exec(this.selection);this.selection=this.selection.replace(/(\n*$)/,"");this.endTag=this.endTag+(match?match[1]:"");match=/(^\n*)/.exec(this.startTag);this.startTag=this.startTag.replace(/(^\n*)/,"");this.before=this.before+(match?match[1]:"");match=/(\n*$)/.exec(this.endTag);this.endTag=this.endTag.replace(/(\n*$)/,"");this.after=this.after+(match?match[1]:"");if(this.before){regexText=replacementText="";while(nLinesBefore--){regexText+="\\n?";replacementText+="\n";}
if(findExtraNewlines){regexText="\\n*";}
this.before=this.before.replace(new re(regexText+"$",""),replacementText);}
if(this.after){regexText=replacementText="";while(nLinesAfter--){regexText+="\\n?";replacementText+="\n";}
if(findExtraNewlines){regexText="\\n*";}
this.after=this.after.replace(new re(regexText,""),replacementText);}};var InputPoller=function(textarea,callback,interval){var pollerObj=this;var inputArea=textarea;var lastStart;var lastEnd;var markdown;var killHandle;this.tick=function(){if(inputArea.selectionStart||inputArea.selectionStart===0){var start,end;if(inputArea.value==""){start=end=0;}else{start=inputArea.selectionStart;end=inputArea.selectionEnd;}
if(start!=lastStart||end!=lastEnd){lastStart=start;lastEnd=end;if(markdown!=inputArea.value){markdown=inputArea.value;return true;}}}
...
return result;};var preprocessMath=function(text){return text.replace(/\\([\(\[])([\s\S]*?)\\([\]\)])/g,function(wholeMatch,open,math,close){return"\\\\"+open+math.replace(/([_\*\\])/g,"\\$1")+"\\\\"+close;});};var makePreviewHtml=function(){if(!wmd.panels.preview&&!wmd.panels.output){return;}
...
if(converter){text=converter.makeHtml(preprocessMath(text));}
var currTime=new Date().getTime();elapsedTime=currTime-prevTime;pushPreviewHtml(text);htmlOut=text;var code_tag=$(wmd.panels.preview).find("code");if(code_tag.length>0){code_tag.replaceWith(function(){return $("<code>"+
$(this).text().replace(/\\\\([\(\[])([\s\S]*?)\\\\([\]\)])/g,function(match){return match.replace(/\\([\(\)\[\]_\*\\])/g,"$1");})+"</code>");});}};var previewRefresh=function(){makePreviewHtml();if(wmd.options.previewRefreshCallback){wmd.options.previewRefreshCallback(wmd);}};var applyTimeout=function(){if(timeout){top.clearTimeout(timeout);timeout=undefined;}
if(startType!=="manual"){var delay=0;if(startType==="delayed"){delay=elapsedTime;}
if(delay>maxDelay){delay=maxDelay;}
timeout=top.setTimeout(previewRefresh,delay);}};var getScaleFactor=function(panel){if(panel.scrollHeight<=panel.clientHeight){return 1;}
...
if(wmd.panels.output){wmd.panels.output.scrollTop=(wmd.panels.output.scrollHeight-wmd.panels.output.clientHeight)*getScaleFactor(wmd.panels.output);}};this.refresh=function(requiresRefresh){if(requiresRefresh){oldInputText="";previewRefresh();}
...
redoButton.execute=function(manager){manager.redo();};var helpButton=createButton("wmd-help-button");helpButton.isHelp=true;setupButton(helpButton,true);buttonRow.appendChild(helpButton);var helpAnchor=document.createElement("a");helpAnchor.href="http://umamao.com/topics/4cbefdba79de4f58ea000116";helpAnchor.target=wmd_options.helpTarget;helpAnchor.title=wmd_options.helpHoverTitle;helpButton.appendChild(helpAnchor);setUndoRedoButtonStates();};var setupEditor=function(){if(/\?noundo/.test(doc.location.href)){wmd.nativeUndo=true;}
